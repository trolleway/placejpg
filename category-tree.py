...
if len(proposeds)>0:
    for proposed_cat in proposeds:
        ...
        text+="{{GeoGroup}}\n"
        text+="[[Category:"+object_commonscat+" in "+str(location_upper_commonscat) +"]]\n"
        if location_commonscat_previous != '':
    ...
    terminal_menu = TerminalMenu(candidates, title="Select number of proposed category for create")
    ...
    modelwiki.create_category(proposed_cats_final[menu_entry_index]['name'],proposed_cats_final[menu_entry_index]['text'])