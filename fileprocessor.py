...
class Fileprocessor:
    ...
    def input2filelist(self, filepath, mode=None):
        if os.path.isfile(filepath):
            ...
            if mode == "replace_duplicates":
                while "commons_duplicates" in folder_keywords:
                    folder_keywords.remove("commons_duplicates")
            files = list(
                filter(
                    lambda name: not any(
                        keyword in name for keyword in folder_keywords
                    ),
                    files,
                )
            )
    ...
    def deprecated_get_wikidata_simplified(self, wikidata) -> dict:
        ...
        if "P373" in object_wd["claims"]:
            ...
            object_record["commons"] = object_wd["sitelinks"]["commonswiki"][
                "title"
            ].replace("Category:", "")
        else:
        ...
        if os.path.basename(filename).find(override_key) > 0:
            ...
            if street_wdid is None:
                ...
                try:
                    ...
                    self.logger.warning(
                        "failed append to not found file, proceed to next gile"
                    )
                return None
    ...
    def make_image_texts_vehicle(
        ...
        colors=None,
        operator=None,
        operator_vehicle_category=None,
        secondary_wikidata_ids=None,
        digital_number=None,
        ...
        if secondary_wikidata_ids is None:
        ...
        if os.path.isfile(country):
            country = self.get_place_from_input(
                filename,
                country,
                geo_dict,
                override_key="_location",
                geodata_attribute="name:en",
            )
            if country is None:
        ...
        if operator is not None:
            ...
            operator_wd = modelwiki.get_wikidata_simplified(
                modelwiki.wikidata_input2id(operator)
            )
        ...
        if system is not None:
            ...
            if "P1813" in system_wd["claims"]:
                for abbr_record in system_wd["claims"]["P1813"]:
                    system_names[abbr_record["language"]] = abbr_record["value"]
        ...
        if number == "BEFORE_UNDERSCORE":
            number = os.path.basename(filename)[
                0 : os.path.basename(filename).find("_")
            ]
    ...
    |description="""
        ...
        if line_wdid is not None:
            assert "en" in modelwiki.get_wikidata_simplified(line_wdid)["labels"], (
                "object https://www.wikidata.org/wiki/"
                + line_wdid
                + " must has name en"
            )
        ...
        if route is not None:
            cat = "{transports} on route {route} in {city}".format(
                transports=transports[vehicle], route=route, city=city_name_en
            )
            cat_content = """
{{GeoGroup}}
...
[[Category:$transports in $city_name_en by route|$route]]"""
            ...
            cat = "Category:$transports in $city_name_en by route".replace(
                "$transports", transports[vehicle]
            ).replace("$city_name_en", city_name_en)
            cat_content = """
{{GeoGroup}}
[[Category:$transports in $city_name_en]]"""
        ...
        cat_content = (
            """{{Usercat}}
[[Category:Photographs_by_"""
            + self.photographer
            + "/"
            + country
            + """]]"""
        )
        modelwiki.create_category(cat, cat_content)
...
[[Category:Photographs_of_"""
        ...
        ), "empty value added to structured data set:" + str(
            " ".join(list(wikidata_4_structured_data))
        )
        ...
        if vehicle in train_synonims:
            ...
            if isinstance(country, str) and len(country) > 3:
                catname = dt_obj.strftime("%B %Y") + " in rail transport in " + country
                categories.add(catname)
                category_content = (
                    "{{GeoGroup|level=2}}{{railtransportmonth-country|"
                    + dt_obj.strftime("%Y")[0:3]
                    + "|"
                    + dt_obj.strftime("%Y")[-1:]
                    + "|"
                    + dt_obj.strftime("%m")
        ...
        if facing is not None:
            ...
            if facing == "Side":
                text += (
                    "[[Category:Side views of " + transports[vehicle].lower() + "]]\n"
                )
            if facing == "Rear":
                text += (
                    "[[Category:Rear views of " + transports[vehicle].lower() + "]]\n"
                )
            if facing == "Front":
                text += (
                    "[[Category:Front views of " + transports[vehicle].lower() + "]]\n"
                )
            if facing == "Rear three-quarter".capitalize():
                text += (
                    "[[Category:Rear three-quarter views of "
            ...
            if facing == "Three-quarter".capitalize():
                text += (
                    "[[Category:Three-quarter views of "
                    + transports[vehicle].lower()
                    + "]]\n"
                )
        ...
        if colors is not None:
            ...
            if colors[0].upper() == "RZDGREEN":
                ...
                text += (
                    "[[Category:{transports} in Nashe Podmoskovie livery]]\n".format(
                        transports=transports_color[vehicle].capitalize(),
                        colorname=colorname,
                    )
                )
                ...
                text += "[[Category:{colorname} {transports}]]\n".format(
                    transports=transports_color[vehicle].lower(), colorname=colorname
                )
        elif colors is None:
            txt = "liveries codes:\n"
        ...
        if number is not None and number.isdigit():
            ...
            if "-" in number_filtered:
                number_filtered = number_filtered[number_filtered.index("-") + 1 :]
            if digital_number is None:
        ...
        if number is not None and vehicle == "bus" and digital_number.isdigit():
            ...
            if not has_category_for_this_vehicle:
                categories.add(catname)
            modelwiki.create_number_on_vehicles_category(
            ...
            if not has_category_for_this_vehicle:
                categories.add(catname)
            modelwiki.create_number_on_vehicles_category(
                vehicle="trolleybus", number=digital_number
            )
            ...
            if cat is not None:
                ...
                if model_wd["commons"] is None:
                    raise ValueError(
                        "vehicle model https://www.wikidata.org/wiki/"
                        + model_wdid
                        + " must has commons category"
                    )
                if vehicle != "auto":
                    categories.discard(model_wd["commons"])  # remove from set if exist
                    categories.add(model_wd["commons"] + "|" + digital_number)
                elif vehicle == "auto":
                    categories.discard(model_wd["commons"])  # remove from set if exist
        ...
        if vehicle == "auto":
            ...
            if subclasses is None:
                self.logger.info(
                    "🤏 subclass of car model in wikidata is null, skipped"
                )
            else:
        ...
        if "system_wd" in locals():
            if (
                vehicle not in train_synonims
                and model_in_location_found == False
                and has_category_for_this_vehicle == False
            ):
            ...
            for wdid in secondary_wikidata_ids:
                ...
                if cat is not None:
                    ...
                    if secondary_objects_should_have_commonscat:
                        assert "commons" in wd_record, (
                            "https://www.wikidata.org/wiki/"
                            + wdid
                            + " must have commons"
                        )
                        assert wd_record["commons"] is not None, (
                            "https://www.wikidata.org/wiki/"
                            + wdid
                            + " must have commons"
                        )
        ...
        for catname in categories:
            ...
            assert catname is not None, (
                "none value in categories:" + str(categories) + " " + filename
            )
        ...
        if os.path.isfile("building-generator.gpkg"):
            ...
            if city_wdid is not None:
                wikidata_4_structured_data.add(city_wdid)
    ...
    def get_wikidatalist_from_string(self, test_str: str) -> list:
        ...
        if "UUID" in test_str:
            ...
            lst = [
                uuids2wikidata[item] if "uuid" in item.lower() else item for item in lst
            ]
    ...
    def get_mapillary_from_string(self, test_str: str) -> list:
        ...
        if "mapillarykey" not in test_str and "mapillaryuser" not in test_str:
            return None, None
    ...
    def get_source_information_part(self, filename):
        ...
        if mapillarykey is None and mapillaryuser is None:
            ...
            text = (
                ...
                + mapillarykey
                + """}}|author= [http://www.mapillary.com/profile/"""
                + mapillaryuser
                + " "
                + mapillaryuser
                + "] @ Mapillary.com"
    ...
    def get_tech_description(self, filename, geo_dict):
        ...
        if any(
            substring in filename.lower()
            for substring in ("stitch", "pano", "photosphere")
        ):
            text = text + "{{Panorama}}" + "\n"
        ...
        if "stitch" in filename or "pano" in filename.lower():
            categories.add(
                "Photographs by " + self.photographer + "/Stitched panoramics"
            )
    ...
    def make_image_texts_simple(
        ...
        if wd_record["commons"] is None:
            self.logger.error(
                "https://www.wikidata.org/wiki/" + wikidata + " must have commons"
            )
            return None
        ...
        if "P31" in wd_record["claims"]:
            for i in wd_record["claims"]["P31"]:
                instance_of_data.append(modelwiki.get_wikidata_simplified(i["value"]))
        ...
        if len(instance_of_data) > 0:
            ...
            for lang in self.langs_primary + self.langs_optional:
                try:
                    objectnames_long[lang] = (
                        ", ".join(
                            d["labels"][lang]
                            for d in instance_of_data
                            if d["labels"][lang].upper()
                            not in objectnames[lang].upper()
                        )
                        + " "
                        + objectnames[lang]
                    )
...
[[Category:Photographs of """
        ...
        for wdid in temp_wikidata_list:
            ...
            if prop in wd["claims"]:
                for claim in wd["claims"][prop]:
                    cat_for_claim = ""
                    cat_for_claim = modelwiki.get_wikidata_simplified(claim["value"])[
                        "commons"
                    ]
    ...
    [[Category:"""
                        ...
                        need_create_categories.append(
                            {"name": cat_for_claim, "content": cat_content}
                        )
...
"""
        ...
        for wdid in temp_wikidata_list:
            ...
            if modelwiki.is_subclass_of_building(wdid) and not modelwiki.is_subclass_of(
                wdid, "Q376799"
            ):
                wd = modelwiki.get_wikidata_simplified(wdid)
                prop = ""
...
"""
        ...
        if rail:
            text += (
                "[[Category:Railway photographs taken on "
        ...
        if "catid2filename" in filename:
            ...
            nam = modelwiki.pagename_from_id(
                caid2filename.replace("catid2filename", "")
            )
    ...
    def renameprocess_filename(
        ...
        commons_filename = (
            ...
            + objectnames["en"]
            + " "
            + dt_obj.strftime(time_suffix_format)
            + suffix
            + filename_extension
        )
        commons_filename = commons_filename.replace("/", " drob ")
        ...
        if add_administrative_name:
            try:
                administrative_name = modelwiki.get_wikidata_simplified(
                    modelwiki.get_upper_location_wdid(
                        modelwiki.get_wikidata_simplified(wikidata)
                    )
                )["labels"]["en"]
    ...
    def get_camera_text(self, filename) -> list:
        ...
        if image_exif.get("make") is not None and image_exif.get("model") is not None:
            if image_exif.get("make") != "" and image_exif.get("model") != "":
                ...
                if (
                    image_exif.get("lensmodel", "") != ""
                    and image_exif.get("lensmodel", "") != ""
                ):
                ...
                if (
                    image_exif.get("fnumber", "") != ""
                    and image_exif.get("fnumber", "") != ""
                    and int(image_exif.get("fnumber", 0)) != 0
                ):
                ...
                if (
                    image_exif.get("'focallengthin35mmformat'", "") != ""
                    and image_exif.get("'focallengthin35mmformat'", "") != ""
                ):
                    st += "|Focal length 35mm = f/" + str(
                        image_exif.get("'focallengthin35mmformat'")
                    )
                st += "}}" + "\n"

                if image_exif.get("usepanoramaviewer") == True:
                    st += "{{Pano360}}" + "\n"
                if (
                    image_exif.get("focallength", "") != ""
                    and image_exif.get("focallength", "") != ""
                    and int(image_exif.get("focallength", 0)) != 0
                ):
                    categories.add(
                        "Lens focal length "
                        + str(round(float(image_exif.get("focallength", 0))))
                        + " mm"
                    )
                    ...
                    try:
                        if int(image_exif.get("iso", 0)) > 49:
                            try:
                                categories.add(
                                    "ISO speed rating "
                                    + str(round(float(str(image_exif.get("iso")))))
                                    + ""
                                )
                            except:
                                self.logger.info(
                                    "ISO value is bad:" + str(image_exif.get("iso", ""))
                                )
                    except:
                        pass
        ...
        with open(path, "rb") as image_file:
            if not path.lower().endswith(".stl"):
                try:
                    ...
                    dt_obj = None
                    cmd = [self.exiftool_path, path, "-datetimeoriginal", "-csv"]
                    if path.lower().endswith((".mp4", ".mov", ".avi", ".mts")):
    ...
    def process_and_upload_files(self, filepath, desc_dict):
        ...
        for filename in files:
            ...
            if secondary_wikidata_ids == [] and (
                "_Q" in filename or "placeQ" in filename or "UUID" in filename
            ):
            ...
            if desc_dict["mode"] in ("object", "addr"):
                if desc_dict["wikidata"] == "FROMFILENAME":
                    if not self.get_wikidatalist_from_string(filename):
                        self.logger.error(
                            filename.ljust(80)
                            + ": no wikidata in filename, skip upload"
                        )
                ...
                texts = self.make_image_texts_vehicle(
                    ...
                    operator_vehicle_category=desc_dict.get(
                        "operator_vehicle_category", None
                    ),
            ...
            if (
                ...
                assert os.path.isdir(
                    desc_dict.get("move_ready", "")
                ), "move_ready folder not exist"
                dest_filepath = os.path.join(
                    desc_dict.get("move_ready", ""), os.path.basename(filename)
                )
                assert not os.path.isfile(
                    dest_filepath
                ), "file already exist in move_ready folder"
            ...
            if (
                filename.endswith((".tif", ".tiff"))
                and src_filesize_mb > self.tiff2webp_min_size_mb
            ):
            ...
            if filename.lower().endswith((".mp4", ".mov", ".avi", ".mts")):
                video_converted_filename = self.convert_to_webm(filename)
            if filename.lower().endswith(
                (".mp4", ".mov", ".avi", ".mts")
            ) and os.path.isfile(video_converted_filename):
                print(
