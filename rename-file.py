...
parser.add_argument(
    ...
    required=False,
    default=2,
    help="1.	At the original uploader’s request, 2 .	To change from a meaningless or ambiguous name to a name that describes what the image particularly displays 3 obvious errors 4  harmonize the names of a set of images 5 violation of Commons’ policies and guidelines 6 Non-controversial maintenance and bug fixes",
)
parser.add_argument(
...
class Helper_rename:
    ...
    def generate_filename(self, pagename, wikidata) -> str:
        ...
        commons_filename = self.fileprocessor.renameprocess_filename(
    ...
    def generate_rename_template(self, new_name: str, rationale: int) -> str:
        ...
        text = (
            ...
            + "|2="
            + str(rationale)
            + "|3=human-readable name from wikidata name}}"
        )
        return text
    ...
    def prepend_text_page(self, pagename, rename_template_text):
        ...
        new_page_text = helper_renamer.prepend_text(
            file_page.text, rename_template_text, "{{Rename"
        )
        if new_page_text is None:
            return None
        page_not_need_change = True
        if new_page_text != file_page.text:
...
if __name__ == "__main__":
    ...
    if pagename is not None:
        ...
        rename_template_text = helper_renamer.generate_rename_template(
            new_name, rationale=args.rationale
        )
        ...
        for change in tqdm(changeset):
            rename_template_text = helper_renamer.generate_rename_template(
                change["to"], rationale=args.rationale
            )
